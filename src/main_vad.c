...
//   vad_data = vad_open(sf_info.samplerate, umbral1, umbral2);
//   /* Allocate memory for buffers */
//   frame_size   = vad_frame_size(vad_data);
//   buffer       = (float *) malloc(frame_size * sizeof(float));
//   buffer_zeros = (float *) malloc(frame_size * sizeof(float));
...
//     /* TODO: print only SILENCE and VOICE labels */
//     /* As it is, it prints UNDEF segments but is should be merge to the proper value */
//    if((last_state_merge!=state) &&(state==M_VOICE || state==M_SILENCE ) && (last_state==ST_SILENCE ||last_state==ST_VOICE)){
...
//       if (t != last_t){
//         if((last_state_merge!=state) && (state==ST_VOICE||state==ST_SILENCE) && (last_state==M_VOICE || last_state==M_SILENCE)){
